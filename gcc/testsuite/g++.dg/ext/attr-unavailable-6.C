...
int func1()
{
   ...
   z = w + x + y + g1 + g2 + g3;	/* { dg-error "'x' is unavailable: You can't use x" "" } */
					/* { dg-error "'y' is unavailable: You can't use y" "y" { target *-*-* } 54 } */
					/* { dg-error "'g2' is unavailable: You can't use g2" "g2" { target *-*-* } 54 } */
					/* { dg-error "'g3' is unavailable: You can't use g3" "g3" { target *-*-* } 54 } */
...
int func2(S1 *p)
{
  ...
  return p->u1.field6 + p->field8;	/* { dg-error "'S1::<anonymous union>::field6' is unavailable: You can't use field6" "" } */
					/* { dg-error "'S1::field8' is unavailable: You can't use field8" "field8" { target *-*-* } 72 } */
}
...
class T {
  public:
    ...
    void member2(INT1) __attribute__ ((__unavailable__("You can't use member2"))); /* { dg-error "'INT1' is unavailable" "" } */
    int member3(T *);
    int x;
