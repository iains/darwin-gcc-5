...
int func1()
{
   ...
   z = w + x + y + g1 + g2 + g3;	/* { dg-error "'x' is unavailable" "" } */
					/* { dg-error "'y' is unavailable" "y" { target *-*-* } 55 } */
					/* { dg-error "'g2' is unavailable" "g2" { target *-*-* } 55 } */
					/* { dg-error "'g3' is unavailable" "g3" { target *-*-* } 55 } */
...
int func2(S1 *p)
{
  ...
  return p->u1.field6 + p->field8;	/* { dg-error "'S1::<anonymous union>::field6' is unavailable" "" } */
					/* { dg-error "'S1::field8' is unavailable" "field8" { target *-*-* } 73 } */
}
...
class T {
  public:
    ...
    void member2(INT1) __attribute__ ((__unavailable__)); /* { dg-error "'INT1' is unavailable" "" } */
    int member3(T *);
    int x;
} __attribute__ ((unavailable));
